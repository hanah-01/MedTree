...
contract EMRTreeLedger {
    ...
    function createGenesisBlock(
        ...
        bytes32 _recordHash
    ) external nonEmptyHash(_recordHash) returns (uint256) {
    ...
    function getBlockDetails(
        ...
        returns (bytes32 blockHash, bytes32 prevHash, string memory metadata, uint256 timestamp)
    {
        Branch storage branch = branches[_branchId];
    ...
    function isPatientRegistered(address _patient) external view returns (bool) {
        return hasGenesisBlock[_patient];
    }
}
